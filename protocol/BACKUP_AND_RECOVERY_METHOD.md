# KMQGB — Backup and Recovery Method

Status: ACTIVE RECOVERY AUTHORITY
Date initialized: 2026-09-08
Repository: `pppuu7-cmd/Known-Models-Quantum-Gravity-Benchmark`
Branch: `main`
Migrated from: `pppuu7-cmd/Relativity-Quantum-Interface-Reconstruction`, branch `rqir7-known-models-benchmark`, source HEAD `782df9af3af1ca55ca4f9bf143e2723a7a8f18bc`.
Original benchmark branch base commit: `02ad31e89f1df0d5515779e6b7526e8eb5505667`.
External Candidate Gravity authority: `pppuu7-cmd/Relativity-Quantum-Interface-Reconstruction/main` and `candidate_gravity/recovery/CURRENT_QG_FRONT.md`; read-only from KMQGB.

## 1. Purpose

This file makes the known-model RQIR benchmark recoverable from a new chat/session without relying on chat memory, hidden state, or prior conversation summaries. All scientific state needed to resume must be reconstructible from this repository plus explicitly recorded read-only RQIR authority references.
...
11. KMQGB writes go only to `pppuu7-cmd/Known-Models-Quantum-Gravity-Benchmark`; the main RQIR repository is read-only for benchmark work.
...
`frozen external RQIR protocol > immutable per-model result/evidence > latest immutable RECOVERY_DELTA > CURRENT_BENCHMARK_FRONT > narrative research_log`
...
Candidate Gravity `main` is an external moving authority. The original benchmark branch base SHA, the migrated source HEAD, and the latest observed Candidate Gravity authority SHA are different concepts and MUST be recorded separately.

## 4. Required artifact map

- `README.md` — scope, queue, status ontology.
- `protocol/FROZEN_RQIR_PROTOCOL_REFERENCE.md` — literal paths/hashes and recovered frozen definitions; unresolved items remain explicitly unresolved.
- `protocol/BACKUP_AND_RECOVERY_METHOD.md` — this recovery method.
- `protocol/KMQGB_CANDIDATE_GRAVITY_SEPARATION.md` — hard repository/firewall rule.
- `models/<model>/audit.md` — human-readable gate-by-gate audit.
- `models/<model>/result.json` — machine-readable result.
- `matrices/benchmark_matrix.csv` — one row per concrete realization.
- `logs/research_log.md` — chronological narrative log.
- `recovery/CURRENT_BENCHMARK_FRONT.md` — concise current frontier.
- `recovery/RECOVERY_DELTA_NNN.md` — immutable per-iteration recovery deltas.
- `recovery/state.json` — machine-readable continuation state.
- `recovery/RESTORE_FROM_NEW_CHAT.md` — shortest fresh-session recovery instructions.
- `MIGRATION_PROVENANCE.md` — source repository/branch/SHA and migrated-file ledger.
...
2. Re-check the latest Candidate Gravity `main` authority SHA/iteration read-only, without silently rebasing KMQGB conclusions.
...
12. Verify that no KMQGB write was made to the external RQIR repository.
...
7. Read `protocol/FROZEN_RQIR_PROTOCOL_REFERENCE.md` and verify every referenced frozen authority path/hash needed for the next gate against the external RQIR repository.
8. Read the current model's `audit.md` and `result.json`.
9. Fetch current external RQIR `main` and `candidate_gravity/recovery/CURRENT_QG_FRONT.md` separately and read-only; do not confuse moving Candidate Gravity authority with KMQGB authority.
10. Execute the `next_actions` field from `state.json` in order.
11. Do not redo completed work unless the recovery check finds corruption, evidence conflict, changed external source, or an explicitly reopened gate.

A minimal prompt in a new chat can therefore be: **'Продолжай KMQGB из репозитория Known-Models-Quantum-Gravity-Benchmark; восстановись строго по recovery/RESTORE_FROM_NEW_CHAT.md.'** No other chat context should be required.
...
- `source_benchmark_head_sha`: migrated source benchmark HEAD;
- `source_branch_base_sha`: original immutable benchmark branch base;
- `candidate_gravity_authority.sha`: latest observed moving external RQIR authority;
- `state_written_from_kmqgb_sha`: KMQGB branch HEAD immediately before writing that state.

After writes complete, fetch the resulting KMQGB `main` HEAD. The next recovery delta records that resulting HEAD as the prior/checkpoint SHA. This avoids circular self-hashing claims.
...
## 12. Migrated recovery checkpoint

Benchmark readiness at source checkpoint: 10% before the source backup iteration.
Current model: 4D Einstein–Hilbert GR, Lambda=0, weak-field Minkowski sector.
Current model check at source checkpoint: 40% before the source backup iteration.
Completed: 0/9.
Rollup: PASS 0; FAIL 0; BLOCKED 0; DEGENERATE 0 final.

Known GR scientific evidence already checked at source checkpoint: Einstein equations admit hyperbolic harmonic formulations; linearized gravitational response admits retarded Green functions; the realization is the intended null control. Exact `EXACT_COMPARATOR_IDENTITY` remains provisional until literal frozen comparator/Q1–Q7 containment is recovered from repository authority.

## 13. Immediate continuation targets

1. Recover literal frozen Q1–Q7 definitions and comparator span from the external RQIR repository; do not infer them.
...
5. Audit external RQIR tree for prior asymptotic-safety/FRG/Reuter comparator work before creating any duplicate audit.
