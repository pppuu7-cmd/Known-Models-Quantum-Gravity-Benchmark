...
def main():
    ...
    recs=[]
    for fn in glob.glob(os.path.join(a.root,'**','iter507-*.json'),recursive=True):
    ...
    found=set(by); missing=sorted(EXPECTED-found); extra=sorted(found-EXPECTED)
        ...
        for lane,v in by.items():
            for q in v.get('radii',[]):
                if abs(float(q.get('eta',-1))-eta) <= eta*1e-12:
                    ...
                    break
        counts[key]=len(vals); min_margin[key]=min(vals) if vals else None
    largest=[float(v.get('largest_certified_eta',0.0)) for v in by.values() if v.get('valid',False)]
    if missing or duplicates or extra or invalid: gate=BLOCK
    elif len(qualified)==18: gate=PASS
    elif sci_fail: gate=FAIL
    else: gate=BLOCK
    common=[eta for eta in ETAS if counts[f'{eta:.0e}']==18]
    out={
      'iteration':507,'lane_count_expected':18,'lane_count_found':len(found),
      ...
      'claim_lock':'No Haar theorem, positive-measure theorem, spectral theorem, D7-S2 closure, terminal D7 classifier, or Candidate Gravity activation.'
    }
