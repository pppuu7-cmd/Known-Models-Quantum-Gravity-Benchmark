...
records=[]; precision_ok=True; stability_ok=True; neg_sep=False; finite=True
for gamma,j in PANEL:
    ...
    for m in range(-j,j+1):
        ...
        for dps in (80,120):
            for b in BETAS:
                tp,tm=tvals(j,m,rho,b,dps); routes[dps].append((tp,tm,tp+tm,tp-tm))
        ...
        labels=('plus','minus','sum')
        powers={}
        for q,label in enumerate(labels):
            mags=[abs(x[q]) for x in routes[120]]
            nonneg=all(v>mp.mpf('1e-30') for v in mags[-4:])
            if nonneg:
                p4=-slope(BETAS[-4:],mags[-4:]); p3=-slope(BETAS[-3:],mags[-3:]); stab=abs(p4-p3)
                stability_ok &= stab<=0.35
                powers[label]={'power_last4':p4,'power_last3':p3,'stability':stab,'status':'CHARACTERIZED'}
            else:
                powers[label]={'status':'NEAR_ZERO_UNRESOLVED'}
