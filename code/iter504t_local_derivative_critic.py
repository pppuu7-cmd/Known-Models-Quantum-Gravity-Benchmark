...
def validate(o,k):
 ...
 for x in leaves:
  ...
  if len(pmrows)!=16:e.append('possible_max_grid')
  for pm in pmrows:
   inds=pm.get('indices',[])
...
def main():
 ...
 out={'gate':GATE,'preregistration_commit':PREREG,'classification':cls,'critic_errors':errs,'cross_environment_exact_decision_agreement':cross,'lane_a_root_sha256':ha,'lane_b_root_sha256':hb,'lane_a_assembled_sha256':aah,'lane_b_assembled_sha256':bbh,'aggregate_sha256':aggh,'negative_controls':neg,'threshold_exact':'1/20','robust_floor_exact':'1','max_depth':MAX_DEPTH,'partition_identity_verified':'exact_rational_dyadic_cells','claim_ceiling':'Independent three-root bounded local-D Critic only'}
