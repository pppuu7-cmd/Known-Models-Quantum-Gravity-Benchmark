...
def main():
 ...
 out={'gate':GATE,'preregistration_commit':PREREG,'classification':cls,'errors':errs,'root_sha256':hs,'root_count':len(roots),'total_terminal_leaves':sum(o.get('terminal_leaf_count',0) for o in roots),'total_unresolved_leaves':unresolved,'total_visited_nodes':sum(o.get('visited_node_count',0) for o in roots),'threshold_exact':'1/20','robust_floor_exact':'1','max_depth':MAX_DEPTH,'scientific_decision_transport':'producer_exact_arb_booleans','roots':roots,'claim_ceiling':'Three-root bounded local-D mechanism test only'}
