...
def derivative_array(dual_arr):
    """Extract the frozen Iter504R derivative component from dual CD channels."""
...
def fixed_channel_rows(perR_channels, candidate_unions):
    ...
    for ir, rho in enumerate(RHOS):
        ...
        all_drift_within = bool(
            rows and complete and mx is not None and float(mx['drift_upper']) <= 0.05
        )
        out.append({
            'rho': rho,
...
def evaluate_root(root_box: int):
    ...
    for loc, aq in point_locations(root_box):
        ...
        for ir, rho in enumerate(RHOS):
            ...
            compF = bool(F['drift_upper'] > 0.05 and FF['competition_test_complete'] and ff_all)
            compC = bool(C['drift_upper'] > 0.05 and CF['competition_test_complete'] and cf_all)
            fixedC = bool(C['drift_upper'] > 0.05 and CF['competition_test_complete'] and not cf_all)
            mech.append({
                'rho': rho,
