...
def center_derivative_array(arr):
...
def envelope_details(vals):
    ...
    for z in a.flat:
        zz = acb(z)
        lo = zz.abs_lower(); up = zz.abs_upper()
    ...
    return {
        'L': L, 'U': U, 'possible': possible,
        'best_lower_index': best, 'dominance_gap': gap,
        'lows': lows, 'ups': ups,
    }
...
def slope_rows(perR):
    ...
    for ir,rho in enumerate(RHOS):
        ...
        rows.append({
            'rho':rho,
            'S_lower':bf(slo,'lower'),'S_upper':bf(shi,'upper'),
            'E_lower':bf(elo,'lower'),'E_upper':bf(ehi,'upper'),
            'drift_upper':bf(du,'upper'),
            'within_tolerance':bool(slo>=FLOOR and du<=TOL),
        })
    return rows
    ...
    for ir,rho in enumerate(RHOS):
        ...
        for ch in candidates:
            ...
            for R in RGRID:
                ...
                lo=q['lows'][ch]; up=q['ups'][ch]
                if not (lo > arb(0) and up.is_finite()):
                    eligible=False; break
                ys[R]=(q['hlog'].lower()+lo.log().lower(), q['hlog'].upper()+up.log().upper())
            if not eligible:
                incomplete.append(ch); continue
            y6l,y6u=ys[6]; y8l,y8u=ys[8]; y10l,y10u=ys[10]; y12l,y12u=ys[12]
            slo=(y12l-y8u)/4; shi=(y12u-y8l)/4
            elo=(y10l-y6u)/4; ehi=(y10u-y6l)/4
            du=max(abs(slo-ehi),abs(shi-elo))
            rows.append({'channel':ch,'drift_upper':bf(du,'upper'),'S_lower':bf(slo,'lower'),'within_tolerance':bool(slo>=FLOOR and du<=TOL)})
        mx=max(rows,key=lambda z:z['drift_upper']) if rows else None
        all_within=bool(rows) and not incomplete and len(rows)==len(candidates) and all(z['within_tolerance'] for z in rows)
        out.append({
            'rho':rho,'candidate_union':candidates,'candidate_count':len(candidates),
            'eligible_count':len(rows),'ineligible_candidates':incomplete,
            'competition_test_complete':not incomplete and len(rows)==len(candidates),
            'all_candidate_fixed_channels_within_tolerance':all_within,
...
def evaluate_treatment(root, delta, collapse_derivative=False):
    perR={}; perR_channels={}; candidate_unions=[set() for _ in RHOS]
    for R in RGRID:
        mr=root['model'][R]
        ...
        for ir,rr in enumerate(mr['rho']):
            dvals=center_derivative_array(rr['dvals']) if collapse_derivative else rr['dvals']
            ...
            ylo=hlog.lower()+ed['L'].log().lower(); yhi=hlog.upper()+ed['U'].log().upper()
            rows.append({
                'rho':rr['rho'],'ylo':ylo,'yhi':yhi,
                'envelope_lower':bf(ed['L'],'lower'),'envelope_upper':bf(ed['U'],'upper'),
                'possible_count':len(ed['possible']),'possible_indices':ed['possible'],
        ...
        perR[R]=rows; perR_channels[R]=chans
    slopes=slope_rows(perR)
    fixed=fixed_channel_rows(perR_channels,candidate_unions)
    compact=[]
    for R in RGRID:
        compact.append({'R':R,'rho':[{
            'rho':q['rho'],'envelope_lower':q['envelope_lower'],'envelope_upper':q['envelope_upper'],
            'possible_count':q['possible_count'],'possible_indices':q['possible_indices'],
            'best_lower_index':q['best_lower_index'],'dominance_gap_lower':q['dominance_gap_lower'],
...
def evaluate_root(root_box:int):
    ...
    for loc,aq in point_locations(root_box):
        ...
        mech=[]
        for ir,rho in enumerate(RHOS):
            F=full['per_rho'][ir]; C=center['per_rho'][ir]
            FF=full['fixed_channel'][ir]; CF=center['fixed_channel'][ir]
            deriv=bool(F['drift_upper']>0.05 and C['drift_upper']<=0.05)
            compF=bool(F['drift_upper']>0.05 and FF['competition_test_complete'] and FF['all_candidate_fixed_channels_within_tolerance'])
            compC=bool(C['drift_upper']>0.05 and CF['competition_test_complete'] and CF['all_candidate_fixed_channels_within_tolerance'])
            fixedC=bool(C['drift_upper']>0.05 and CF['competition_test_complete'] and not CF['all_candidate_fixed_channels_within_tolerance'])
            mech.append({
                'rho':rho,'full_D_within_tolerance':F['within_tolerance'],
        ...
        cases.append({
            'location':loc,'amp_q':qt(aq),'delta_q':qt(aq-root['midq']),
            'full_D':full,'center_D_sensitivity':center,'mechanism_flags':mech,
        })
    return {
        'gate':GATE,'preregistration_commit':PREREG,'root_box':root_box,
        'causal':'0to5','block':0,'path':2,'direction':direction,'sign':sign,
        'precision_bits':384,'full_channel_count':243,'channel_pruning_used':False,
        'root_model_build_count':1,'derivative_recomputed_at_points':False,
        'root_controls':root['controls'],'min_beta_lower':root['min_beta_lower'],
        'cases':cases,
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--root-box',type=int,required=True,choices=ROOTS); ap.add_argument('--out',required=True); a=ap.parse_args()
    try:
        out=evaluate_root(a.root_box)
    except Exception as e:
        out={'gate':GATE,'preregistration_commit':PREREG,'root_box':a.root_box,'invalid':True,'error':repr(e)}
