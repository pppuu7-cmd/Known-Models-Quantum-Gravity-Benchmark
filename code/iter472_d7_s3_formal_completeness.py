...
bool_re=re.compile(r'(?P<key>[A-Za-z0-9_\- ]{0,90})(?:[:=])\s*(?P<val>True|False|true|false)')
for fn in files:
    if not fn.endswith(('.py','.json','.md','.yml','.yaml')): continue
    ...
    for cls,toks in TOKENS.items():
        ...
        for m in bool_re.finditer(text):
            frag=(m.group('key')+' '+m.group(0)).lower()
            if any(t.lower() in frag for t in toks):
                explicit.append({'key':m.group('key').strip(),'value':m.group('val').lower()=='true'})
        state='MENTION_ONLY'
        if explicit:
...
# synthetic controls, never evidence
...
known_negative=any(x['file']==known and x['state'] in ('NEGATIVE','CONTRADICTORY') for cls in findings.values() for x in cls)
summary={}
...
checks={'synthetic_positive_detected':ctrl_pos,'synthetic_negative_detected':ctrl_neg,'known_negative_recovered':known_negative,'mention_only_not_positive':all(x['state']!='POSITIVE' or x['explicit'] for v in findings.values() for x in v)}
