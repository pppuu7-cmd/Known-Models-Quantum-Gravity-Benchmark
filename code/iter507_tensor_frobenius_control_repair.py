...
PASS = 'ITER507_TENSOR_FROBENIUS_CONTROL_REPAIR_CERTIFIED_SCOPED'
FAIL = 'SCIENTIFIC_FAIL_ITER507_NO_CERTIFIED_OPEN_NEIGHBORHOOD_AT_FROZEN_RADII'
BLOCK = 'BLOCKED_OR_INFRASTRUCTURE_ITER507'


def tensor_frobenius(T):
    a = np.asarray(T)
...
def evaluate(panel, causal, rho):
    ...
    tensor_prefactor = float(np.prod([tensor_frobenius(ts[channel[v]]) for v in range(5)]))
    ...
    checks = {
        ...
        'tensor_norm_repair_exact_entrywise_euclidean': True,
    }
    ...
    return {
        ...
        'scope': 'Iter506 control-only tensor-Frobenius repair on fixed-causal j=1 full-K5 leading contraction; same frozen radii and science thresholds; no Haar, spectral pairing, physical-vertex, D7-S2 closure, or terminal-classifier claim',
    }
...
def main():
    ...
    try:
        ...
        out = {'iteration':507,'lane':f'{a.panel}-{a.causal}-rho{a.rho}','panel':a.panel,'causal':a.causal,'rho':a.rho,'valid':False,'pass':False,'classification':BLOCK,'error':repr(e)}
